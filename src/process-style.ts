 ...
 */
export const processNodePseudoStyle = (
  original: HTMLElement,
  clone: HTMLElement,
) => {
  if (!(clone instanceof Element)) return clone
  const eachPseudoStyles = () =>
    [':before', ':after'].forEach((item) => nodePseudoStyle(item))
  /**
   * 处理伪类
  ...
  return Promise.resolve()
    .then(() =>
      setCloneNodeStyleProperty(window.getComputedStyle(original), clone.style),
    )
...
function getFormatPseudoStyle(
  ...
  if (style.cssText) {
    ...
    for (const key of style) {
      cssText += `${key}:${style.getPropertyValue(key)}${
